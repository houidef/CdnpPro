...
begin//0
	...
	sub_004CCCB8;
	f2D8 := Msg.message;
    NbPeriodes := FichierCdn.sub_004BE9E0();//get Nbre Periodes
    if ( f2D8 <= NbPeriodes) then //004CB557
    ...
    begin//2
		...
		Cells[lvar_4 + 4 + K + NbPeriodes, 0 ] := 'Moyenne annuelle sur ' + IntToStr(sub_004B9E48);
		...
		sub_004CD0B0(lvar_4 + NbPeriodes + 3 + K);
	end//2
	else 
	begin
		if (f2D8 <= NbPeriodes) then
		...
		begin
			...
			begin//2
				...
				sub_004CD0B0(NbPeriodes + 2);
				sub_004CD0B0(NbPeriodes + 3);
				lvar_4 := lvar_4 + lvar_18;
			end;//2
			Cells[lvar_4 + 3 + NbPeriodes, 0 ] := 'Moyenne annuelle sur ' + IntToStr(sub_004B9E48); 
			...
			sub_004CD0B0(lvar_4 + NbPeriodes + 2);
			Cols[lvar_4 + 4 + NbPeriodes].Clear;
			Cols[lvar_4 + 5 + NbPeriodes].Clear;
			Cols[lvar_4 + 5 + NbPeriodes] := FichierCdn.sub_004C5078(NbPeriodes + 1, sub_004B9EA0, sub_004B9E10, sub_004BC298, sub_004BC30C);
			sub_004CD0B0(lvar_4 + 4 + NbPeriodes);
		end;
	end;
end;//0
  ...
  //004CCCB8
    ...
    Cols[0].Clear;
      ...
      begin//004CCE66
        ...
        FichierCdn.sub_004BEA64(I, lvar_4);
        FichierCdn.sub_004C8BB8(I, Buf);
        if ((Trim(Buf) <> '') and sub_004BB9B4) then //004CCEF8
          begin//004CCF01
            FichierCdn.sub_004C8BB8(I, Buf);
            lvar_4 := lvar_4 + ' (' + Buf + ')';
          end;//5
        if (FichierCdn.sub_004C8E50(I) and sub_004BB95C) then //004CCF70
          //004CCF79
            lvar_4 := lvar_4 + ' (R)';
        Cells[1 , I] :=  lvar_4;
      end;//3
...
begin//0
  //004CD7E0
    Appreciations := FichierCdn.sub_004C8AE8; //Nbr Appreciations
    if (Visible = False) then Exit;
    if (ACol <> 0) then
    begin//004CD864
      if (ARow = 0) then
    ...
    begin//004CD988
      ...
      begin//3 //004CD99E
        ...
        Canvas.Font.Color := 0;
        if (f2D8 <= FichierCdn.sub_004BE9E0) then//004CDA00
    ...
    if (f2D8 <= FichierCdn.sub_004BE9E0) then//004CDD33 Nbre Periodes
